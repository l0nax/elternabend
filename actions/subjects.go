...
// Model: Singular (Subject)
// DB Table: Plural (subjects)
// Resource: Plural (Subjects)
// Path: Plural (/subjects)
// View Template Folder: Plural (/templates/subjects/)

// SubjectsResource is the resource for the Subject model
type SubjectsResource struct{
  buffalo.Resource
}

// List gets all Subjects. This function is mapped to the path
// GET /subjects
func (v SubjectsResource) List(c buffalo.Context) error {
...
func (v SubjectsResource) Show(c buffalo.Context) error {
  // Get the DB connection from the context
  tx, ok := c.Value("tx").(*pop.Connection)
  if !ok {
    return fmt.Errorf("no transaction found")
...
func (v SubjectsResource) New(c buffalo.Context) error {
  return c.Render(200, r.Auto(c, &models.Subject{}))
}
// Create adds a Subject to the DB. This function is mapped to the
// path POST /subjects
func (v SubjectsResource) Create(c buffalo.Context) error {
  // Allocate an empty Subject
  subject := &models.Subject{}
...
func (v SubjectsResource) Edit(c buffalo.Context) error {
  // Get the DB connection from the context
  tx, ok := c.Value("tx").(*pop.Connection)
  if !ok {
    return fmt.Errorf("no transaction found")
...
func (v SubjectsResource) Update(c buffalo.Context) error {
  // Get the DB connection from the context
  tx, ok := c.Value("tx").(*pop.Connection)
  if !ok {
    return fmt.Errorf("no transaction found")
...
func (v SubjectsResource) Destroy(c buffalo.Context) error {
  // Get the DB connection from the context
  tx, ok := c.Value("tx").(*pop.Connection)
  if !ok {
    return fmt.Errorf("no transaction found")
